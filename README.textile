...
*Simply copy the last 4 lines that are outputted into your config.rb file*
...
First make sure you have set your DISCOVERY_URL and DISCOVERY_CAPABILITY in config.rb to match the server you are trying to connect
to, and then run either bin/bot or bin/bot_cluster; bin/bot will ask you which class of bot you want to run, while bin/bot_cluster
...
To create a tournament, you need to use the admin tool.  The easiest way to use it is from IRB.
...
This will create a tournament that starts at 5:22 PM with blinds starting at 1 and increasing every 10 minutes.
